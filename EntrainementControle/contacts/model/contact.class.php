...
class Contact implements JsonSerializable
{
  ...
  public function create() : void
  {
    ...
    if ($this->id !== -1) {
      throw new Exception("Create impossible : déjà dans la base avec cet id=" . $this->id);
        ...
        var_dump($nom);
    $dao->exec("INSERT INTO contacts (prenom, nom, mobile) VALUES (?,?,?)", [$this->prenom, $this->nom, $this->mobile]);
    }
    // 
    ///////////////////////////////////////////////////////
    //  A COMPLETER
  ...
  public static function read(int $id): Contact
  {
      $dao = DAO::get();
      try{
          $result = $dao->query("SELECT * FROM contact WHERE id = ?", [$id]);
          if(empty($result)){
              echo "Pas d'élément récupérés !";
              return new Contact();
          }else{
              $result = $result[0];
              return new Contact($result["nom"], $result["prenom"], $result["mobile"]);
          }
